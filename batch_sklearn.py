...
def add_tasks(batch_service_client, job_id, input_files, source_files, output_container_sas_url):
    """
    ...
    for idx, input_file in enumerate(input_files):
        ...
        output_file_path = "".join((input_file_path).split('.')[-2]) + '.output'
        command = "/bin/bash -c \""\
...
def _upload_source_files(
    ...
    source_code_paths = []
    for folder, _, files in os.walk(os.path.join(sys.path[0], config._APP_PATH)):
    ...
    return [
        upload_file_to_container(blob_client, container_name, file_path, config._APP_PATH) for file_path in source_code_paths
    ]
...
if __name__ == '__main__':
    ...
    try:
        ...
        add_tasks(batch_client, config._JOB_ID, input_files,
                  source_files, output_container_sas_url)
